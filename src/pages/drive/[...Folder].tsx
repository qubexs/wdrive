...
function Folder() {
  ...
  const { list, loading: listLoading } = useFetchFiles(
    currentFolderId,
    session?.user.id ?? "",
    session?.user.email ?? undefined,
  );
  const { entries: allFiles, loading: allLoading } = useFetchAllFiles(
    session?.user.id ?? "",
    session?.user.email ?? undefined,
  );
                  ...
                  {isFolder && (
                    ...
                    <div className="mb-5 flex flex-col space-y-4">
                      ...
                      <div className={containerCls}>
                        <GetFolders folderId={currentFolderId} select={""} view={view} />
                      </div>
                    </div>
                  )}
                  {isFile && (
                    ...
                    <div className="mb-5 flex flex-col space-y-4">
                      ...
                      <div className={containerCls}>
                        <GetFiles folderId={currentFolderId} select={""} view={view} />
                      </div>
                    </div>
                  )}
                </>
