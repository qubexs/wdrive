...
export default async function handler(
  ...
  const canUpload = (session as any)?.user?.canUpload !== false || (session as any)?.user?.role === "ADMIN" || session.user.email === "demo@local.dev";
  if (!canUpload) return res.status(403).json({ error: "Upload permission denied" });
