...
const isAdminLike = (session: any) =>
  session?.user?.role === "ADMIN" ||
  session?.user?.email === "demo@local.dev";

/**
 * Shared Drive API.
 * - GET: any active user can list sharedDrive entries (read-only for non-admin).
 * - POST: admin only. Creates folder/file metadata with sharedDrive=true.
 */
export default async function handler(
  ...
  if ((session as any)?.user?.isActive === false)
    return res.status(403).json({ error: "Account disabled" });

  if (req.method === "GET") {
    const entries = await db.fileEntry.findMany({
      where: { sharedDrive: true } as any,
  ...
  if (!isAdminLike(session))
    return res.status(403).json({ error: "Only admin can add to shared Drive" });
  ...
  if (folderId) {
    const parent = await db.fileEntry.findFirst({
      where: { id: folderId, isFolder: true, sharedDrive: true } as any,
      select: { id: true },
    });
    if (!parent)
  ...
  if (!isFolder) {
    // quota counts against the uploading admin
    const limit =
      (session as any)?.user?.storageLimitBytes !== undefined
        ? (session as any)?.user?.storageLimitBytes
        : USER_STORAGE_LIMIT_BYTES;
    const usage = await db.fileEntry.aggregate({
      where: { ownerId: userId, isFolder: false },
      _sum: { fileSize: true },
    });
    if (limit !== null && (usage._sum.fileSize ?? 0) + fileSize > limit) {
