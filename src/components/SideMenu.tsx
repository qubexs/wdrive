...
function SideMenu() {
  ...
  const isAdminLike = role === "ADMIN" || userEmail === "demo@local.dev";
  ...
  const { entries: allFiles } = useFetchAllFiles(userId ?? "", userEmail ?? undefined);
  ...
  // null = unlimited
  const storageLimitBytes = (session?.user as any)?.storageLimitBytes !== undefined ? (session?.user as any)?.storageLimitBytes : USER_STORAGE_LIMIT_BYTES;

  const ensureWithinStorageLimit = (bytesToAdd: number) => {
    if (currentUsageBytes + bytesToAdd > storageLimitBytes) {
      window.alert(
        `You have reached your ${formatBytes(storageLimitBytes)} storage limit.`,
      );
      return false;
    }
  ...
  return (
    <section className="relative h-[90vh] w-16 space-y-4 duration-500 tablet:w-60">
      <button
        onClick={() => (canUpload ? setIsDropDown(true) : requireUpload())}
        title={canUpload ? "New" : "New (no upload permission)"}
        className={`mt-1 flex w-fit items-center justify-center space-x-2
      rounded-2xl bg-white p-3 text-textC shadow-md shadow-[#ddd]
      duration-300 hover:bg-darkC2 hover:shadow-[#bbb] tablet:px-5 tablet:py-4 ${canUpload ? "" : "opacity-50"}`}
      >
        ...
        <div className="rounded-2xl bg-white/90 px-2 py-3 text-sm text-textC shadow-sm shadow-[#ddd] tablet:px-4">
          <div className="mb-2 flex items-center justify-between font-medium">
            ...
            <span>
              ...
              {storageLimitBytes === null ? "∞" : formatBytes(storageLimitBytes)}
            </span>
          </div>
          <div className="hidden h-2 overflow-hidden rounded-full bg-darkC2 tablet:block">
            <div
              className={`h-full rounded-full ${storageLimitBytes !== null && currentUsageBytes >= storageLimitBytes / 1.25 ? "bg-red-500" : "bg-[#1a73e8]"}`}
              style={{
                width: `${storageLimitBytes === null ? 0 : Math.min(
                  100,
